...
let chats = []; let currentChatId = null; let apiKey = localStorage.getItem('apiKey') || "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";
...
fetch("https://openrouter.ai/api/v1/models") .then(response => {
if (!response.ok) { console.error("مشكلة في اتصال OpenRouter API"); }
else { console.log("اتصال API يعمل بشكل صحيح"); } }) .catch(error => console.error("فشل الاتصال بـ OpenRouter:", error)); }
...
const langMatch = block.match(/(\w*)\n/);   const lang = langMatch ? langMatch[1] : 'text';   const codeText = block.replace(/(\w*)\n/, '').replace(/```$/, '');
const codeBlock =   <div style="position: relative; margin: 15px 0;">   <div class="code-header">   <span class="language-tag">${lang}</span>   <button class="copy-btn" onclick="copyCode(this)">   <i class="fas fa-copy"></i> نسخ الكود   </button>   </div>   <pre><code class="language-${lang}">${escapeHtml(codeText)}</code></pre>   </div>  ;
...
div.innerHTML =    <div class="message-header">   <div class="message-sender">   ${msg.role === 'user' ? 'أنت' : 'Taiz Soft'}   </div>   <div class="message-time">${formatTime(msg.timestamp)}</div>   </div>   <div class="message-content">${formatMessageContent(msg.content)}</div>;
}
...
li.innerHTML =   <div class="chat-info">   <i class="fas fa-comment chat-icon"></i>   <span>${chat.title}</span>   </div>   <button class="delete-chat" onclick="deleteChat(event, '${chat.id}')">   <i class="fas fa-trash"></i>   </button>  ;
